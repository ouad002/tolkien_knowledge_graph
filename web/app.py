...
def entity_exists_in_kg(entity_uri):
...
def entity(entity_name):
    ...
    # Check if entity exists in KG first
    if not entity_exists_in_kg(entity_uri):
        # Redirect to Tolkien Gateway if entity not found
        wiki_url = f"https://tolkiengateway.net/wiki/{entity_name.replace('_', ' ')}"
        return redirect(wiki_url, code=303)
...
def serve_html(entity_uri, entity_name):
    ...
    if not results or not results['results']['bindings']:
        # Should not happen since we check existence before, but safety fallback
        wiki_url = f"https://tolkiengateway.net/wiki/{entity_name.replace('_', ' ')}"
        return redirect(wiki_url, code=303)
    ...
    for binding in results['results']['bindings']:
        ...
        if 'type' in prop_uri:
            ...
            continue  # Don't show rdf:type in properties
        
        # Track label
        if 'label' in prop_uri:
            entity_label = value
            continue  # Don't show rdfs:label as a property
        
        # Track source
        if 'source' in prop_uri:
            entity_source = value
            continue  # Handle source separately
        
        # Skip internal properties we don't want to display
        if any(x in prop_uri for x in ['#type', '/label']):
            continue
        ...
        # Determine if value is internal KG URI or external
        is_internal_uri = is_uri(value) and value.startswith(BASE_URI)
        
        # Better label for URIs
        display_label = value_label
        if not display_label:
            if is_internal_uri:
                display_label = value.split('/')[-1].replace('_', ' ')
            else:
                display_label = value
        ...
        properties[prop_name].append({
            ...
            'label': display_label,
            'is_uri': is_uri(value),
            'is_internal': is_internal_uri,
            'local_name': value.split('/')[-1] if is_internal_uri else None
        })
